...
; This test exercises the lowering of the intrinsic @llvm.umul.with.overflow.i32
; to the UMul DXIL op. UMul returns the full 64-bit product split into its low
; and high 32-bit halves (%dx.types.twoi32 = { i32, i32 }), so the overflow bit
; is synthesized as (high != 0).

; CHECK-DAG: [[DX_TYPES_TWOI32:%dx\.types\.twoi32]] = type { i32, i32 }

define noundef i32 @test_UMul(i32 noundef %a, i32 noundef %b) {
; CHECK-LABEL: define noundef i32 @test_UMul(
; CHECK-SAME: i32 noundef [[A:%.*]], i32 noundef [[B:%.*]]) {
; CHECK-NEXT:    [[UMUL:%.*]] = call [[DX_TYPES_TWOI32]] @dx.op.binaryWithTwoOuts.i32(i32 42, i32 [[A]], i32 [[B]])
; CHECK-NEXT:    [[LOW:%.*]] = extractvalue [[DX_TYPES_TWOI32]] [[UMUL]], 0
; CHECK-NEXT:    [[HIGH:%.*]] = extractvalue [[DX_TYPES_TWOI32]] [[UMUL]], 1
; CHECK-NEXT:    [[OVF_BIT:%.*]] = icmp ne i32 [[HIGH]], 0
...
;
  %umul = call { i32, i1 } @llvm.umul.with.overflow.i32(i32 %a, i32 %b)
  %prod = extractvalue { i32, i1 } %umul, 0
  %ovf = extractvalue { i32, i1 } %umul, 1
  %ovf_zext = zext i1 %ovf to i32
  %result = add i32 %prod, %ovf_zext
  ret i32 %result
}

define noundef <2 x i32> @test_UMul_vec2(<2 x i32> noundef %a, <2 x i32> noundef %b) {
; CHECK-LABEL: define noundef <2 x i32> @test_UMul_vec2(
...
; CHECK-NEXT:    [[UMUL_I0:%.*]] = call [[DX_TYPES_TWOI32]] @dx.op.binaryWithTwoOuts.i32(i32 42, i32 [[A_I0]], i32 [[B_I0]])
; CHECK-NEXT:    [[LOW_I0:%.*]] = extractvalue [[DX_TYPES_TWOI32]] [[UMUL_I0]], 0
; CHECK-NEXT:    [[HIGH_I0:%.*]] = extractvalue [[DX_TYPES_TWOI32]] [[UMUL_I0]], 1
; CHECK-NEXT:    [[OVF_BIT_I0:%.*]] = icmp ne i32 [[HIGH_I0]], 0
...
; CHECK-NEXT:    [[UMUL_I1:%.*]] = call [[DX_TYPES_TWOI32]] @dx.op.binaryWithTwoOuts.i32(i32 42, i32 [[A_I1]], i32 [[B_I1]])
; CHECK-NEXT:    [[LOW_I1:%.*]] = extractvalue [[DX_TYPES_TWOI32]] [[UMUL_I1]], 0
; CHECK-NEXT:    [[HIGH_I1:%.*]] = extractvalue [[DX_TYPES_TWOI32]] [[UMUL_I1]], 1
; CHECK-NEXT:    [[OVF_BIT_I1:%.*]] = icmp ne i32 [[HIGH_I1]], 0
...
;
  %umul = call { <2 x i32>, <2 x i1> } @llvm.umul.with.overflow.v2i32(<2 x i32> %a, <2 x i32> %b)
  %prod = extractvalue { <2 x i32>, <2 x i1> } %umul, 0
  %ovf = extractvalue { <2 x i32>, <2 x i1> } %umul, 1
